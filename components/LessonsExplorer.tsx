...
import { loadLessonTheoryDetails } from "../lib/lesson-theory-details";
...
export function LessonsExplorer({lessons,initialTheory}:{lessons:LessonViewModel[];initialTheory:LessonDeepTheory}){
  ...
  useEffect(()=>{
    if(!lessonId||deepTheory)return;
    let active=true;
    queueMicrotask(()=>{
      if(active)clearTheoryErrorFor(lessonId);
    });
    loadLessonTheoryDetails(lessonId)
      .then((theory)=>{
        if(active)setTheoryById((current)=>({...current,[lessonId]:theory}));
      })
      .catch(()=>{
        if(active){
          setTheoryError({
            lessonId,
            message:"Không tải được lý thuyết của bài này. Hãy kiểm tra kết nối rồi thử lại.",
          });
        }
      });
    return()=>{
  ...
  },[clearTheoryErrorFor,deepTheory,lessonId,theoryRetry]);
  ...
  const pick=(id:string)=>{clearTheoryErrorFor(id);setSelectedId(id);setSection("overview");syncUrl(id,"push")};
